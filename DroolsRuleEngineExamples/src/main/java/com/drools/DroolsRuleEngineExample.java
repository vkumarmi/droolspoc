...
public class DroolsRuleEngineExample {
    ...
    public static final void main(String[] args) {
        try {
            ...
            KieSession kSession = kContainer.newKieSession("poc-rules"); 
            scenario1(kSession);
           // scenario2(kSession);
        } catch (Throwable t) {
            t.printStackTrace();
    ...
    public static void scenario1(KieSession kSession)
    {
    ...
    private static void scenario2(KieSession kieSession)
    {
		...
		p5.setAvailableQty(2);
 		...
 		List<CartItem> cartItems = newCustomer.getCart().getCartItems();
 		for (CartItem cartItem: cartItems) {
 			kieSession.insert(cartItem);
 		}
 		kieSession.insert(newCustomer.getCart());
 		//kieSession.setGlobal("outOfStockProducts", new ArrayList<Product>());
 		System.out.println("************* Fire Rules **************");
